...
export function SkillBadge({ label }: SkillBadgeProps) {
  ...
  if (reduceMotion) {
    return (
      <span className="px-4 py-2 rounded-lg border border-border bg-card/50 text-sm font-medium hover:bg-accent/10 hover:border-accent hover:text-accent transition-colors cursor-default inline-block">
        {label}
      </span>
    );
  }
